...
class DataLoaderCustom(Dataloader):
    ...
    def __init__(self, path_dataset):
        super(DataLoaderCustom, self).__init__(path_dataset=path_dataset)
        # self.init_phase('all')

    def init_phase(self, phase):
        print('init phase: {}'.format(phase))
        assert phase in ['train', 'valid', 'test', 'all']
        adj_file = os.path.join(self.path_dataset, 'adj_{}.npz'.format(phase))
        feats_file = os.path.join(self.path_dataset,'features_{}.npz'.format(phase))
        np.random.seed(1234)
    ...
    #                     [index, index_answer])  # append the edge
    #                 if i == 0:
    ...
    #     assert len(flat_questions) == len(gt) and len(
    #         q_ids) == len(gt) and len(gt) == len(valid)
        ...
        for question in questions:
            ...
            for index in outfit_subset:  # indexes of outfit nodes
                ...
                for index_answer in question[1]:
                    ...
                    # the correct connection is the first
                    gt.append(int(i == 0))
                    # a link is valid if the candidate item is from the same category as the missing item
                    valid.append(int(question[2][i] == question[3]))
                    i += 1
    ...
    def setup_test_compatibility(self, filename='compatibility_test.json', resampled=False):
        """
        """
        comp_file = os.path.join(self.path_dataset, filename)
        if resampled:
            comp_file = os.path.join(self.path_dataset, 'compatibility_RESAMPLED_test.json')
