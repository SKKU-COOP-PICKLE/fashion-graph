...
from dataloaders import DataLoaderPolyvore, DataLoaderCustom
...
ap.add_argument("-d", "--dataset", type=str, default="custom",
                choices=['fashiongen', 'polyvore', 'amazon', 'custom'],
                help="Dataset string.")
...
# prepare data_loader
if DATASET in ['polyvore', 'custom']:
    if DATASET == 'polyvore':
        dl = DataLoaderPolyvore(os.path.join(DATASET_DIR, DATASET, 'dataset'))
    elif DATASET == 'custom':
        dl = DataLoaderCustom(os.path.join(DATASET_DIR, DATASET, 'dataset'))
...
if DATASET != 'amazon' and DATASET != 'custom':
